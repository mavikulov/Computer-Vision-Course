...
def quantize():
    ...
    checkpoint = torch.load("fp32_model.pt", weights_only=True)
    ...
    torch.save(quantized_model_pytorch.state_dict(), "quantized_model.pt")
...
def load_quantized_model_pytorch():
    ...
    # load weights
    quantized_model_pytorch = quantize_fx.convert_fx(model_prepared)
    checkpoint = torch.load("quantized_model.pt", weights_only=True)
    quantized_model_pytorch.load_state_dict(checkpoint)
