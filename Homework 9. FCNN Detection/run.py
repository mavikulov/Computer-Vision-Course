...
def grade(data_path):
    ...
    passed_iou = results[1]["status"] == "Ok"
    passed_auc = results[2]["status"] == "Ok"
    passed_nms = results[4]["status"] == "Ok"

    # You must pass the IoU and AUC tests to qualify for the Detector tests
    if not (passed_iou and passed_auc):
        results[3]["status"] = "Failed"
        results[5]["status"] = "Failed"

    # You must pass the NMS tests to qualify for the Detector+NMS test
    if not passed_nms:
        results[5]["status"] = "Failed"
